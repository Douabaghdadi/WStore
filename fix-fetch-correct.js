...
files.forEach(file => {
  try {
    ...
    // Pattern 1: fetch(` avec backtick mal fermé - remplacer par fetch(`...`)
    // Chercher tous les fetch(` qui se terminent par ")
    content = content.replace(/fetch\(`([^`]+?)"\)/g, 'fetch(`$1`)');
    
    // Pattern 2: Corriger les Authorization: `Bearer ${token}` qui sont cassés
    content = content.replace(/Authorization:\s*`Bearer\s*\$\{token\}`/g, 'Authorization: `Bearer ${token}`');
